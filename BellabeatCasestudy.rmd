...
In this task I were given a public data set that explores smart device users' daily habits that is [FitBit Fitness Tracker Data](https://www.kaggle.com/datasets/arashnic/fitbit) and was stored on Kaggle by Mobius with [CC0 liscense](https://creativecommons.org/publicdomain/zero/1.0/)

But after explorer, we figure out that the data is copyright with [CCBY - Attribution](https://creativecommons.org/licenses/by/4.0/legalcode) license, and original stored on [Zenodo](https://zenodo.org/record/53894#.YoD7HNpBy02), so if we use this data, we are required attribution to the creator and include the BY element.

So we will use data in the original that is stored on Zenodo and cite as Furberg, R., Brinton, J., Keating, M., & Ortiz, A. (2016). Crowd-sourced Fitbit datasets 03.12.2016-05.12.2016 [Data set]. Zenodo. [\@furberg2016](https://doi.org/10.5281/zenodo.53894)\
...
After downloading data, we got two compressed files were named by two periods of time, first is from 03/12/2016 to 04/11/2016 and another is from 04/12/2016 to 05/12/2016
...
It's look like some of data from first period time are missing. We will explore that later on, first we are going to import and merge data that are available in whole period time

But first we need to install and load packages that are the collection of R functions, complied code and sample data to made easier and faster data process

## 2.2 Packages & library

-   The Core `tidyverse` package include essential packages inside that are:
    -   `readr` to quickly importing CSV file
    -   `tidyr` to tidying data that is a way to organize tabular data in a consistent data structure across packages. consistent data structure across packages. A table is tidy if
    -   `dplyr` to transforming data
    -   `stringr` to working with character type as string
    -   `ggplot2` to visualize data
-   The `janitor` package to cleaning data
...
Let's install package and load library.
...
Let the data is integrity that must conform principals are accuracy, completeness, consistency and trustworthiness throughout its life cycle

1.  Accuracy is the degree to which the data conforms to the actual entity being measured or described, so we will check the relation of variable in the merging table in each hierarchy of time from minute to hourly to daily

2.  Completeness is the degree to which the data contains all desired components or measures, so we will check missing data in minute, hourly and daily and effect of it to data validity
...
4.  And trustworthiness is check for bias and credibility in the data that are Reliable, Original, Comprehensive, Current and Cited

    -   Reliable: Due to sample size is 30 that is the smallest sample size for which the Central Limit Theorem is still valid, but in this sample size is not having any demographic information so we could encounter a sampling bias
    -   Original: Due to this data were generated by third party provider - respondents to a distributed survey via Amazon Mechanical Turk so this data is not original from fitbit
    -   Comprehensive: Parameters match most of Bellabeat products' parameters
...
-   Sample size of 30 FitBit users with no any demographic information so that are not sure if the sample is representative of the population as a whole.
-   As data is collected in a survey, we are unable to ascertain it's integrity or accuracy.
...
We need to clean data that we could manipulate it much more easier.

Before clean data, we need to create `dataList` vector because we will use `for()` loop many time
...
Our data name is in `camelCase` and variables are still in `CamelCase`, so it hard to recognize, so we will change variable name to `snake_case` using janitor::clean_names()

Let's see how `clean_names` function change variable names
...
Notice that the `DATETIME` is used in this data is 12-hours format and that is recognized as character type, we need to convert it into 24-hours format and `DATETIME` type

We need to convert data as `DATE` format
...
for(i in dataList){
  ...
  eval(parse(text = i))
  ##Process counter loading... --%
  a = round(str_which(dataList, a)/length(dataList)*100, 1)
  print(str_c("loading...", a))
}
...
We got the `dailyActivity` data include of - `total_steps` - `very_active_minutes` - `sedentary_minutes` - `lightly_active_minutes` - `fairly_active_minutes` - `very_active_minutes` - `calories` that have been grouped by `id`, `activity_date`, and the activity `intensity` level
...
I already uploaded the Bellabeat data presentation in Tableau [Link](https://public.tableau.com/app/profile/phatlv/viz/BellabeatCasestudy_16564986351180/Story1)

In the presentation includes the analysis report, final conclusions and recommendations based on analysis
